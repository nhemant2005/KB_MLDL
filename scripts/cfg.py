...
# ── Model ──────────────────────────────────────────────────────────────────────
MODEL: str = _user.get("model", "claude-sonnet-4-6")
...
BOOK_PATHS: dict = {k: v["path"] for k, v in _user_books.items()} if _user_books else _arch.get("BOOK_PATHS", {})

# Chapter page map: user overrides arch
CHAPTER_PAGE_MAP: dict = _user.get("chapter_page_map", _arch.get("CHAPTER_PAGE_MAP", {}))
...
PROMPT_CACHING:           bool = bool(_gen.get("prompt_caching", False))
